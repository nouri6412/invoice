...
class Admin_Woo_Invoice_Models_List extends WP_List_Table
{
    ...
    function get_sql($per_page = 5, $page_number = 1, $all_data = false)
    {
        ...
        $data = mbm_invoice\tools::get_sql($per_page, $page_number, $all_data, $this->model_table_name, $this->data_model["fields"], $this->where);
    ...
    {
        _e('نتیجه ای یافت نشد', 'admin-woo');
    }
    ...
    function column_id($item)
    {
        ...
        if (!isset($this->model_obj["is_report"])) {
            $actions = [
                ...
                'edit'   => sprintf('<a onclick="admin_woo_model_form(\'%s\',%d);" type="button" data-bs-toggle="modal" data-bs-target="#invoice-model-form" href="#">ویرایش</a>', esc_attr($this->model_obj["name"]), esc_attr(absint($item[$this->primary_key])))
            ];
        }
