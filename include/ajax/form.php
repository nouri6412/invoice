<?php
class Admin_Woo_Invoice_Ajax_Form
{
    function form()
    {
        global $wpdb;
        ...
        $model = $Admin_Woo_Invoice_Models->get_model($model_name);
    ...
    function field_select_auto($field)
    {
        ...
        foreach ($items  as $item) {
            ...
            // }
            $ret .= sprintf('<div target-id="%s" onclick="invoice_auto_select_item(jQuery(this));" class="auto-select-item" %s value="%s" title="%s" >%s</div>', esc_html($field["title"]), esc_attr($selected), esc_attr($item[$field["type"]["select"]["key"]]), esc_html($item[$field["type"]["select"]["label"]]), esc_html($item[$field["type"]["select"]["label"]]));
        }
    ...
    function field_date($field)
    {
        ...
        if (strlen($values["value"]) == 0) {
            $values["value"] = mbm_invoice\tools::to_shamsi(date('Y-m-d', strtotime(date("Y-m-d") . ' - 0 days')));
        } else {
            $arr = explode("/", $values["value"]);
            if (count($arr) == 1) {
                $values["value"] = mbm_invoice\tools::to_shamsi($values["value"]);
            }
        }
    ...
    {
        global $wpdb, $Admin_Woo_Invoice_Core;
        $title = "";
        $is_true = true;
        $primary_value = 0;
        $primary_key = '';
        ...
        foreach ($model["fields"] as $field) {
            if (isset($field["in_form"]) && $field["in_form"]) {
                if (isset($_POST[$field["title"]])) {
                    ...
                    // $Admin_Woo_Invoice_Core->add_alert($field["title"] . " " . $_POST[$field["title"]], "success");
                    ...
                    if (isset($field["is_primary"]) && $field["is_primary"]) {
                        ...
                        $value = mbm_invoice\tools::to_miladi($value);
                    ...
                    if (isset($field["is_require"]) && $field["is_require"] && strlen($value) == 0) {
                        if (isset($field["type"]) && isset($field["type"]["type"]) && $field["type"]["type"] != "hidden") {
                            $is_true = false;
                            $Admin_Woo_Invoice_Core->add_alert($field["label"] . " " . "نباید خالی بماند", "danger");
                        }
                    }
                }
            }
        ...
        if ($is_true) {
            ...
            if ($primary_value == 0) {
                ...
                if ($insert_id > 0) {
                    ...
                    $Admin_Woo_Invoice_Core->add_alert("با موفقیت ثبت شد " . " " . $insert_id, "success");
                } else {
                    $Admin_Woo_Invoice_Core->add_alert("خطا در ثبت اطلاعات دوباره امتحان فرمائید " . " " . $insert_id, "danger");
                }
            } else {
                ...
                //  if($query_result>0)
                {
                    ...
                    $Admin_Woo_Invoice_Core->add_alert("با موفقیت ثبت شد " . " " . $query_result, "success");
                }
            }
        }
    }
...
add_action('wp_ajax_admin_woo_model_form', array($Admin_Woo_Invoice_Ajax_Form, 'form'));
